...
const Answers = props => {
    ...
    const [currentAnswers, setCurrentAnswers] = useState(initialAnswersState);
    ...
    const getAnswers = questionId => {
        AnswerService.getAnswersByQuestionId(questionId)
            .then(response => {
                setCurrentAnswers(response.data);

                console.log(response.data);
                console.log('currentQuestion line 77 response.data',response.data)
    ...
    useEffect(() => {
        getAnswers(props.match.params.id);
